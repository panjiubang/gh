...
const BeijingClock = () => {
  ...
  useEffect(() => {
    const iv = setInterval(() => {
      ...
      setTime(`北京时间 ${String(bj.getUTCHours()).padStart(2,'0')}:${String(bj.getUTCMinutes()).padStart(2,'0')}:${String(bj.getUTCSeconds()).padStart(2,'0')}`);
    }, 1000);
    return () => clearInterval(iv);
  }, []);
  return <span className="text-[var(--cyan-primary)] font-bold tabular-nums">{time || '北京时间 --:--:--'}</span>;
};
...
export default function Dashboard() {
  ...
  const handleMouseCoords = useCallback((coords: { lat: number; lng: number }) => {
    ...
    geocodeTimer.current = setTimeout(async () => {
      ...
      } catch (e) { console.warn('[观寰] Suppressed error:', e instanceof Error ? e.message : e); }
  ...
  const handleRightClick = useCallback(async (coords: { lat: number; lng: number }) => {
    ...
    } catch (e) { console.warn('[观寰] Suppressed error:', e instanceof Error ? e.message : e); } finally { setDossierLoading(false); }
  }, []);
  ...
  const fetchEndpoint = useCallback(async (
    ...
    try {
      ...
      console.warn('[观寰] Suppressed error:', e instanceof Error ? e.message : e);
  ...
  useEffect(() => {
    ...
    return loadCameraCatalog(cameras => {
      ...
      setBackendStatus('connected');
    }, () => console.warn('[观寰] Camera catalogue load failed; bounded retry scheduled'));
  }, [activeLayers.cctv]);
  ...
  useEffect(() => {
    ...
    // Satellite Ships (from MySQL AIS)
    if (activeLayers.satellite_ships && !layerFetchedRef.current.has('satellite_ships')) {
      fetchEndpoint('/api/satellite-ships', d => ({ satellite_ships: d }));
      layerFetchedRef.current.add('satellite_ships');
    }
    // Balloons
  ...
  // ── 观寰 SDK — Intelligence Fusion Layer ──
    ...
    <main className="fixed inset-0 w-full h-full bg-[var(--bg-void)] overflow-hidden">
      ...
      <AnimatePresence>
        {showSplash && (
          ...
          >
            ...
            {/* ── 观寰 title — letter-by-letter stagger ── */}
            <div className="flex items-center gap-[2px] mb-3 z-[2]">
              {'观寰'.split('').map((letter, i) => (
                <motion.span
                  key={i}
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 3 }} className="status-bar-desktop absolute top-4 right-6 z-[200] pointer-events-none flex items-center gap-3 text-[10px] font-mono tracking-widest text-[var(--text-muted)]">
        ...
        <span className="hidden lg:inline-flex items-center gap-1" title="Tracked entities on map">
          ...
          <span className="opacity-60">个目标</span>
        </span>
      </motion.div>
