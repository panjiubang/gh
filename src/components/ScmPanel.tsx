    ...
    <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.7, duration: 0.6 }} className="glass-panel p-3 pointer-events-auto mt-3 border border-[#00BCD4]/30" style={{ background: 'rgba(0, 188, 212, 0.05)' }}>
      <button onClick={() => setExpanded(!expanded)} className="flex items-center justify-between w-full mb-2">
        <div className="flex items-center gap-2">
          ...
          <span className="hud-text text-[11px] text-[var(--text-primary)]">供应链风险指挥中心</span>
          {totalRisks > 0 && (
            <span className="gotham-tag gotham-tag--critical" style={{ fontSize: '9px', padding: '1px 4px' }}>{totalRisks} 警报</span>
          )}
        </div>
        <div className="flex items-center gap-2">
            ...
            <div className="space-y-3 mt-2 overflow-y-auto styled-scrollbar pr-1 flex-1 pb-4">
              ...
              {marketAlerts.length > 0 && (
                <div>
                  <div className="flex items-center gap-1.5 mb-1.5">
                    ...
                    <span className="text-[10px] font-mono text-[#FF9500] tracking-widest font-bold">市场影响警报</span>
                  </div>
              ...
              <div>
                <div className="flex items-center gap-1.5 mb-1.5">
                  ...
                  <span className="text-[10px] font-mono text-[var(--text-muted)] tracking-widest">关键供应商</span>
                </div>
                {criticalSuppliers.length === 0 ? (
                  <div className="text-[10px] font-mono text-[#00E676] px-2">✓ 所有监控中的一级/二级节点运行正常。</div>
                ) : (
                  <div className="space-y-1">
              ...
              <div>
                <div className="flex items-center gap-1.5 mb-1.5">
                  ...
                  <span className="text-[10px] font-mono text-[var(--text-muted)] tracking-widest">拥堵节点</span>
                </div>
                {(congestedPorts.length === 0 && riskyChokes.length === 0) ? (
                  <div className="text-[10px] font-mono text-[#00E676] px-2">✓ 全球海运流通最优。</div>
                ) : (
                  <div className="space-y-1">
                    {riskyChokes.map((c: any, i: number) => (
                      <div key={`c-${i}`} className="px-2 py-1.5 rounded hover:bg-white/5 transition-colors border-l-2" style={{ borderLeftColor: c.risk === 'CRITICAL' ? '#FF1744' : '#FF9500' }}>
                        <div className="flex justify-between items-center mb-0.5">
                          ...
                          <span className="text-[9px] font-mono font-bold px-1 rounded" style={{ background: c.risk === 'CRITICAL' ? '#FF1744' : '#FF9500', color: '#000' }}>{c.risk}</span>
                        </div>
                    ...
                    {congestedPorts.map((p: any, i: number) => (
                      <div key={`p-${i}`} className="px-2 py-1.5 rounded hover:bg-white/5 transition-colors border-l-2" style={{ borderLeftColor: p.congestion === 'SEVERE' ? '#FF1744' : '#FF9500' }}>
                        <div className="flex justify-between items-center mb-0.5">
                          ...
                          <span className="text-[9px] font-mono font-bold px-1 rounded" style={{ background: p.congestion === 'SEVERE' ? '#FF1744' : '#FF9500', color: '#000' }}>{p.congestion}</span>
                        </div>
                        <div className="flex justify-between items-center text-[9px] font-mono text-[#aaa]">
                          <span>停靠时长：<span className="text-[#fff]">{p.dwell_time}</span></span>
